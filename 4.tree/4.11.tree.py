...
class BTNode:
    ...
    def calc_height(self, n):
        ...
        if hLeft > hRight:
            ...
            return hRight + 1
    def isLeaf(self):
        """이 노드가 단말 노드(Leaf Node)인지 확인합니다."""
        # 왼쪽 자식과 오른쪽 자식이 모두 없으면 True를 반환합니다.
...
def buildETree(exp):
    #중위표기법(ex. A+B)을 후위 표기법(1, 3, +, 4, 2, /, *)으로 변경해서 출력
    if len(exp) == 0:
        return None
    token = exp.pop()
    if token in "+-/*":
        node = BTNode(token)
        node.right = buildETree(exp) #pop 요소를 제외하고 뒤에서 부터 
        node.left = buildETree(exp) #pop 요소를 제외하고 뒤에서 부터 
        return node
    else:
